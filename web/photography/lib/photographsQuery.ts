...
/**
 * Read paginated photograph rows from Supabase (browser or SSR).
 *
 * Uses public `NEXT_PUBLIC_SUPABASE_*` credentials; table RLS must allow
 * anonymous read for the gallery.
 *
 * @param limit - Maximum number of rows to return (1–100).
 * @param offset - Number of rows to skip.
 * @returns Paginated rows and total count; `null` when Supabase is
 *     unconfigured or the query errors. Empty tables return
 *     `{ photos: [], total: 0 }`.
 */
