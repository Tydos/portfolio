import { getSession } from "./auth";
import {
  queryHeroFeaturedPhotographs,
  queryPhotographsFromSupabase,
} from "./photographsQuery";
import { isSupabaseConfigured } from "./supabase";
import { HERO_NATURE } from "../constants/config";
import type { Photo } from "../types";
import type { PhotographRow } from "./photographsQuery";

/** Default page size for public gallery fetches. */
export const PHOTOS_PAGE_SIZE = 25;

/** Why a gallery fetch returned no rows (distinct from an empty table). */
export type GalleryReadError = "not_configured" | "query_failed";

export interface FetchPhotosResult {
  photos: Photo[];
  total: number;
  error?: GalleryReadError;
}

const emptyResult: FetchPhotosResult = { photos: [], total: 0 };

/** Maps a Supabase photograph row into the gallery `Photo` shape. */
const toPhoto = (row: PhotographRow): Photo => ({
  id: row.id,
  src: row.url,
  width: row.width || 2000,
  height: row.height || 2000,
  title: row.filename || "Photo",
  category: row.category || "",
});

/**
 * Resolves the FastAPI base URL used for admin upload/delete.
 *
 * Uses `NEXT_PUBLIC_API_URL` when set. On the photography Vercel project
 * (FastAPI at `/api`), defaults to same-origin `/api` in the browser.
 *
 * @returns Base URL without a trailing slash.
 * @throws If no base can be resolved (e.g. SSR without env).
 */
function getMutationApiBase(): string {
  const fromEnv = process.env.NEXT_PUBLIC_API_URL?.replace(/\/$/, "") ?? "";
  if (fromEnv) {
    return fromEnv;
  }
  if (typeof window !== "undefined") {
    return `${window.location.origin}/api`;
  }
  throw new Error(
    "NEXT_PUBLIC_API_URL is not set. Admin upload/delete require the FastAPI base URL.",
  );
}

/**
 * Reads the admin Bearer token from the current Supabase session.
 *
 * @returns Access token for authorized mutation requests.
 * @throws If the user is not signed in.
 */
async function getAdminAccessToken(): Promise<string> {
  const { data } = await getSession();
  const token = data.session?.access_token;
  if (!token) {
    throw new Error("Sign in required for photo upload/delete.");
  }
  return token;
}

/** Returns true when `name` ends with `.jpg` or `.jpeg` (case-insensitive). */
function isJpegFilename(name: string): boolean {
  const lower = name.toLowerCase();
  return lower.endsWith(".jpg") || lower.endsWith(".jpeg");
}

/**
 * Extracts a human-readable error message from a failed API response.
 *
 * @param res - Failed `fetch` response.
 * @returns FastAPI `detail` string when present, otherwise status text.
 */
async function readErrorDetail(res: Response): Promise<string> {
  try {
    const body: unknown = await res.json();
    if (body && typeof body === "object" && "detail" in body) {
      const detail = (body as { detail: unknown }).detail;
      if (typeof detail === "string") return detail;
    }
  } catch {
    /* ignore non-JSON */
  }
  return res.statusText || `HTTP ${res.status}`;
}

/**
 * Fetch a page of photographs for the public gallery via Supabase.
 *
 * Never throws; returns an empty list on failure.
 *
 * @param page - 1-based page index (default 1).
 * @param pageSize - Maximum number of photos to return (default
 *     {@link PHOTOS_PAGE_SIZE}).
 * @returns Photos plus total count for pagination.
 */
/**
 * Fetch nature photographs for the hero strip (category or curated ids).
 *
 * @returns Up to {@link HERO_NATURE.limit} photos; empty when none match.
 */
export async function fetchHeroNaturePhotos(): Promise<Photo[]> {
  if (!isSupabaseConfigured()) {
    return [];
  }

  const rows = await queryHeroFeaturedPhotographs({
    categories: HERO_NATURE.categories,
    photoIds: HERO_NATURE.photoIds,
    limit: HERO_NATURE.limit,
  });

  if (!rows) {
    return [];
  }

  return rows.map(toPhoto);
}

export const fetchPhotos = async (
  page = 1,
  pageSize = PHOTOS_PAGE_SIZE,
): Promise<FetchPhotosResult> => {
  try {
    if (!isSupabaseConfigured()) {
      console.warn(
        "fetchPhotos: set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY in web/photography/.env.local",
      );
      return { ...emptyResult, error: "not_configured" };
    }

    const offset = (page - 1) * pageSize;
    const result = await queryPhotographsFromSupabase(pageSize, offset);
    if (!result) {
      return { ...emptyResult, error: "query_failed" };
    }
    return {
      photos: result.photos.map(toPhoto),
      total: result.total,
    };
  } catch (err) {
    console.warn(
      "fetchPhotos:",
      err instanceof Error ? err.message : "unknown error",
    );
    return { ...emptyResult, error: "query_failed" };
  }
};

/**
 * Upload a JPEG photograph through FastAPI `POST /upload`.
 *
 * Requires a signed-in Supabase session and a reachable FastAPI base URL.
 *
 * @param file - Image file; must be `.jpg` or `.jpeg`.
 * @param category - Category label stored with the photograph.
 * @throws If the file type is invalid, env/session is missing, or the API
 *     rejects the upload.
 */
export const uploadPhoto = async (
  file: File,
  category: string,
): Promise<void> => {
  if (!isJpegFilename(file.name)) {
    throw new Error("Only .jpg/.jpeg files are accepted.");
  }

  const base = getMutationApiBase();
  const token = await getAdminAccessToken();
  const filename = `${Date.now()}_${file.name.toLowerCase()}`;

  const body = new FormData();
  body.append("file", file, filename);
  body.append("category", category);

  const res = await fetch(`${base}/upload`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
    body,
  });

  if (!res.ok) {
    throw new Error(await readErrorDetail(res));
  }
};

/**
 * Delete a photograph through FastAPI `DELETE /delete/{id}`.
 *
 * Requires a signed-in Supabase session and a reachable FastAPI base URL.
 *
 * @param id - Database id of the photograph to remove.
 * @throws If env/session is missing or the API rejects the delete.
 */
export const deletePhoto = async (id: number): Promise<void> => {
  const base = getMutationApiBase();
  const token = await getAdminAccessToken();

  const res = await fetch(`${base}/delete/${id}`, {
    method: "DELETE",
    headers: { Authorization: `Bearer ${token}` },
  });

  if (!res.ok) {
    throw new Error(await readErrorDetail(res));
  }
};
