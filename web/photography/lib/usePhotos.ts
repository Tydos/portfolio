 ...
 *
 * Public reads query Supabase from the browser; upload/delete call FastAPI with
...
export function usePhotos(pageSize = PHOTOS_PAGE_SIZE) {
  ...
  /**
   * Load a specific page of photos from Supabase.
   *
  ...
  const goToPage = useCallback(
    async (p: number) => {
      ...
      try {
        const { photos: data, total: t, error } = await fetchPhotos(
          p,
          pageSize,
        );
        setPhotos(data);
