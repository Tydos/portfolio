...
import { PORTFOLIO_URL } from "../constants/config";
              ...
              {showUpload && (
                ...
                >
                  ...
                  {actionStatus && !uploadProgress && (
                    <p
                      className={`text-xs ${actionStatus.ok ? "text-emerald-400" : "text-rose-400"}`}
                    >
                      {actionStatus.msg}
                    </p>
                  )}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-slate-600">
            ...
            <p className="text-sm uppercase tracking-widest">
              No photos yet — start the API and upload some
            </p>
          </div>
        ) : (
          <>
              ...
              <div className="mt-10 flex flex-col items-center gap-3">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    disabled={page === 1}
                    className="px-3 py-1.5 rounded border border-slate-700 text-slate-400 hover:border-slate-500 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed text-xs transition-colors"
                  >
                    ←
                  </button>
                  {Array.from(
                    ...
                    <button
                      key={p}
                      onClick={() => goToPage(p)}
                      className={`px-3 py-1.5 rounded border text-xs transition-colors ${p === page ? "border-rose-500 text-rose-400 bg-rose-500/10" : "border-slate-700 text-slate-500 hover:border-slate-500 hover:text-white"}`}
                    >
                      {p}
                    </button>
                  ))}
