...
import { Camera, Search, ChevronLeft, ChevronRight } from "react-feather";
...
function Portfolio() {
  ...
  return (
    <>
      <div className="max-w-6xl mx-auto px-6 mb-8 flex items-baseline justify-between gap-4">
        <SectionTitle className="mb-0">Portfolio</SectionTitle>
        {total > 0 && (
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="mb-6 relative max-w-sm">
          <Search size={15} className="absolute left-0 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
          <input
            type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search photos…"
            className="w-full bg-transparent border-b border-slate-200 pl-6 pr-0 py-2 text-sm text-ink placeholder-ink-muted/60 focus:outline-none focus:border-ink transition-colors"
          />
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 px-6 text-ink-muted">
          ...
          <p className="text-base">{photos.length === 0 ? "No photos to show yet." : "No photos match your search."}</p>
          <p className="text-sm mt-2">{photos.length === 0 ? "Check back soon for new work." : "Try a different keyword or clear the filters."}</p>
        </div>
      ) : (
        <>
          <div className="max-w-6xl mx-auto px-6">
